...
int main(int argc, char **argv) {
  ...
  if (argc == 1) {
    ...
    printf(
        "\t\tUpper bound on vocabulary size, i.e. keep the <int> most frequent "
        "words. The minimum frequency words are randomly sampled so as to "
        "obtain an even distribution over the alphabet.\n");
    printf("\t-min-count <int>\n");
    printf(
        "\t\tLower limit such that words which occur fewer than <int> times "
        "are discarded.\n");
    printf("\nExample usage:\n");
    printf(
        "./vocab_count -verbose 2 -max-vocab 100000 -min-count 10 < corpus.txt "
        "> vocab.txt\n");
    return 0;
  }
