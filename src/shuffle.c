...
/* Merge shuffled temporary files; doesn't necessarily produce a perfect
 * shuffle, but good enough */
int shuffle_merge(int num) {
  ...
  for (fidcounter = 0; fidcounter < num; fidcounter++) {
    // num = number of temporary files to merge
  ...
  while (1) {  // Loop until EOF in all files
    i = 0;
    // Read at most array_size values into array, roughly array_size/num from
    // each temp file
    for (j = 0; j < num; j++) {
...
int shuffle_by_chunks() {
  ...
  while (1) {  // Continue until EOF
    if (i >= array_size) {
      // If array is full, shuffle it and save to temporary file
      shuffle(array, i - 2);
      l += i;
  ...
  fclose(fid);
  free(array);
  // Merge and shuffle together temporary files
...
int main(int argc, char **argv) {
  ...
  if (argc == 1) {
    ...
    printf(
        "\t\tLimit to length <int> the buffer which stores chunks of data to "
        "shuffle before writing to disk. \n\t\tThis value overrides that which "
        "is automatically produced by '-memory'.\n");
    ...
    printf(
        ...
        "'coccur')\n");
    printf(
        "./shuffle -verbose 2 -memory 8.0 < cooccurrence.bin > "
