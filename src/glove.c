...
int write_header = 1;  // 0=no, 1=yes; writes vocab_size/vector_size as first
                       // line for use with some libraries, such as gensim.
...
int use_binary = 0;    // 0: save as text files; 1: save as binary; 2: both. For
                       // binary, save both word and context word vectors.
int model = 2;  // For text file output only. 0: concatenate word and context
                // vectors (and biases) i.e. save everything; 1: Just save word
                // vectors (no bias); 2: Save (word + context word) vectors (no
                // biases)
int checkpoint_every = 0;  // checkpoint the model for every checkpoint_every
                           // iterations. Do nothing if checkpoint_every <= 0
real eta = 0.05;           // Initial learning rate
real alpha = 0.75, x_max = 100.0;  // Weighting function parameters, not
                                   // extremely sensitive to corpus, though may
                                   // need adjustment for very small or very
                                   // large corpora
                                   //
// W 是词向量和上下文向量，gradsq 是 Adagrad 的累积梯度
real *W, *gradsq, *cost;
...
void initialize_parameters() {
  ...
  /* Allocate space for word vectors and context word vectors, and correspodning
   * gradsq */
  // Might perform better than malloc
  a = posix_memalign((void **)&W, 128,
                     2 * vocab_size * vector_size * sizeof(real));
  if (W == NULL) {
    fprintf(stderr, "Error allocating memory for W\n");
    exit(1);
  }
  // gradsq 是 Adagrad 的累积梯度
...
void *glove_thread(void *vid) {
  ...
  for (a = 0; a < lines_per_thread[id]; a++) {
    ...
    for (b = 0; b < vector_size; b++) {
      ...
      gradsq[b + l1] += temp1 * temp1;  // Adagrad 累积梯度平方和
...
int save_params(int nb_iter) {
  /*
   * nb_iter is the number of iteration (= a full pass through the cooccurrence
   * matrix).
   * nb_iter > 0 => checkpointing the intermediate parameters, so nb_iter is
   * in the filename of output file.
    ...
    if (use_unk_vec) {
      ...
      for (a = vocab_size - num_rare_words; a < vocab_size; a++) {
        for (b = 0; b < (vector_size + 1); b++) {
          ...
          unk_context[b] +=
              W[(vocab_size + a) * (vector_size + 1) + b] / num_rare_words;
        }
      }
...
int train_glove() {
  ...
  // Assuming the file isn't corrupt and consists only of CREC's
  ...
  for (b = 0; b < num_iter; b++) {
    ...
    long long *thread_ids =
        (long long *)malloc(sizeof(long long) * num_threads);
    ...
    if (checkpoint_every > 0 && (b + 1) % checkpoint_every == 0) {
      fprintf(stderr, "    saving itermediate parameters for iter %03d...",
              b + 1);
      save_params_return_code = save_params(b + 1);
      if (save_params_return_code != 0) return save_params_return_code;
      fprintf(stderr, "done.\n");
    }
  }
  free(pt);
...
int main(int argc, char **argv) {
  ...
  if (argc == 1) {
    ...
    printf(
        "\t\tIf 1, write vocab_size/vector_size as first line. Do nothing if 0 "
        "(default).\n");
    printf("\t-vector-size <int>\n");
    printf(
        "\t\tDimension of word vector representations (excluding bias term); "
        "default 50\n");
    printf("\t-threads <int>\n");
    ...
    printf("\t-binary <int>\n");
    printf(
        "\t\tSave output in binary format (0: text, 1: binary, 2: both); "
        "default 0\n");
    printf("\t-model <int>\n");
    printf(
    ...
    printf(
        "\t\t   2: output word vectors + context word vectors, excluding bias "
        "terms\n");
    printf("\t-input-file <file>\n");
    printf(
        "\t\tBinary input file of shuffled cooccurrence data (produced by "
        "'cooccur' and 'shuffle'); default cooccurrence.shuf.bin\n");
    ...
    printf(
        "\t\tFilename, excluding extension, for squared gradient output; "
        "default gradsq\n");
    printf("\t-save-gradsq <int>\n");
    printf(
        "\t\tSave accumulated squared gradients; default 0 (off); ignored if "
        "gradsq-file is specified\n");
    printf("\t-checkpoint-every <int>\n");
    printf("\t\tCheckpoint a  model every <int> iterations; default 0 (off)\n");
    printf("\nExample usage:\n");
    printf(
        "./glove -input-file cooccurrence.shuf.bin -vocab-file vocab.txt "
        "-save-file vectors -gradsq-file gradsq -verbose 2 -vector-size 100 "
        "-threads 16 -alpha 0.75 -x-max 100.0 -eta 0.05 -binary 2 -model "
        "2\n\n");
    result = 0;
  } else {
    if ((i = find_arg((char *)"-write-header", argc, argv)) > 0)
      write_header = atoi(argv[i + 1]);
    ...
    if ((i = find_arg((char *)"-checkpoint-every", argc, argv)) > 0)
